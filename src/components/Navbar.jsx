          ...
          <div className="mt-3 grid flex-1 grid-cols-1 gap-2"> 
                {navLinks.map((link) => {
                  ...
                  if (link.path === "/achievements") {
                    return (
                      <div key={link.path}>
                        <button onClick={() => setMobileAchOpen(!mobileAchOpen)} className={`w-full flex items-center justify-between relative rounded px-2 py-2 text-left text-xs font-medium uppercase tracking-[0.06em] transition-all duration-300 ${isActive ? 'bg-[#facc15] text-white' : 'bg-[#f6efe2] text-[#2b0a14] hover:bg-[#facc15] hover:text-white transform transition-transform duration-150 hover:scale-115'}`}>
                        ...
                        {mobileAchOpen && (
                          <div className="mt-1 space-y-1">
                            {achievementsSub.map((s) => (
                              <Link key={s.path} to={s.path} onClick={() => { handleNavClick(); setMobileAchOpen(false); }} className={`ml-4 block rounded px-2 py-2 text-left text-xs font-medium uppercase tracking-[0.06em] transition-all duration-300 ${normalizePath(pathname) === normalizePath(s.path) ? 'bg-[#facc15] text-white' : 'bg-[#f6efe2] text-[#2b0a14] hover:bg-[#facc15] hover:text-white transform transition-transform duration-150 hover:scale-105'}`}>
                                {s.label}
                              </Link>
                            ))}
                          </div>
                  ...
                  if (link.path === "/media") {
                    return (
                      <div key={link.path}>
                        <button onClick={() => setMobileMediaOpen(!mobileMediaOpen)} className={`w-full flex items-center justify-between relative rounded px-2 py-2 text-left text-xs font-medium uppercase tracking-[0.06em] transition-all duration-300 ${isActive ? 'bg-[#facc15] text-white' : 'bg-[#f6efe2] text-[#2b0a14] hover:bg-[#facc15] hover:text-white transform transition-transform duration-150 hover:scale-105'}`}>
                          <span>{link.label}</span>
                        ...
                        {mobileMediaOpen && (
                          <div className="mt-1 space-y-1">
                            {mediaSub.map((s) => (
                              <Link key={s.path} to={s.path} onClick={() => { handleNavClick(); setMobileMediaOpen(false); }} className={`ml-4 block rounded px-2 py-2 text-left text-xs font-medium uppercase tracking-[0.06em] transition-all duration-300 ${normalizePath(pathname) === normalizePath(s.path) ? 'bg-[#facc15] text-white' : 'bg-[#f6efe2] text-[#2b0a14] hover:bg-[#facc15] hover:text-white transform transition-transform duration-150 hover:scale-105'}`}>
                                {s.label}
                              </Link>
                            ))}
                          </div>
